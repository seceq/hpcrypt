...
impl Signature {
    ...
    pub fn from_der(der: &[u8]) -> Result<Self, CurveError> {
        ...
        let total_len = Self::read_der_length_p256(der, &mut pos)?;
        ...
        // Decode r
        let r = Self::decode_der_integer_p256(der, &mut pos)?;

        // Decode s
        let s = Self::decode_der_integer_p256(der, &mut pos)?;
    ...
    /// Read a DER length field with strict validation for P-256
    fn read_der_length_p256(der: &[u8], pos: &mut usize) -> Result<usize, CurveError> {
        if *pos >= der.len() {
            return Err(CurveError::InvalidSignature);
        }
        ...
        *pos += 1;
        ...
        // Long form - not expected for P-256 signatures (max ~72 bytes)
        ...
        // For P-256, if long form is used, it must be necessary (length >= 128)
    ...
    /// Decode a DER INTEGER with strict validation for P-256
    fn decode_der_integer_p256(der: &[u8], pos: &mut usize) -> Result<[u8; 32], CurveError> {
        if *pos + 2 > der.len() {
            return Err(CurveError::InvalidSignature);
        }
        ...
        *pos += 1;
        ...
        // Read length (P-256 integers are small, so only short form expected)
        if der[*pos] & 0x80 != 0 {
            // Long form length - not expected for P-256 integers
            return Err(CurveError::InvalidSignature);
        }
...
impl VerifyingKey {
    ...
    pub fn verify(&self, message: &[u8], signature: &Signature) -> bool {
        ...
        // Since p > n for P-256, we must explicitly reduce
