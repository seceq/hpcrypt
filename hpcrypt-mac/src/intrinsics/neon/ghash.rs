//! ARM NEON + PMULL optimized GHASH implementation using R/F Algorithm
//!
...
/// GHASH state using ARM NEON + PMULL with R/F algorithm
pub struct GhashNeon {
    key: GhashNeonKey,
    /// Current accumulator
    acc: uint64x2_t,
...
#[inline]
unsafe fn byte_swap(x: uint8x16_t) -> uint8x16_t {
    // Reverse bytes: vrev64q reverses within each 64-bit lane
    // Then swap the two 64-bit lanes
    let rev = vrev64q_u8(x);
    let swapped: uint64x2_t = vreinterpretq_u64_u8(rev);
    let swapped = vextq_u64(swapped, swapped, 1);
    vreinterpretq_u8_u64(swapped)
}
...
unsafe fn mulx(h: uint64x2_t) -> uint64x2_t {
    ...
    let reduction_vec = vcombine_u64(vcreate_u64(reduction), vcreate_u64(0));
    veorq_u64(shifted, reduction_vec)
}
...
impl GhashNeonKey {
    ...
    pub unsafe fn new(h: &[u8; 16]) -> Self {
        ...
        let h_raw = vld1q_u8(h.as_ptr());
        let h_reversed = byte_swap(h_raw);
        let h_reversed = vreinterpretq_u64_u8(h_reversed);
...
impl GhashNeon {
    ...
    /// Requires NEON and AES/PMULL support
    #[target_feature(enable = "neon", enable = "aes")]
    pub unsafe fn from_key(key: GhashNeonKey) -> Self {
        Self {
            key,
            acc: vdupq_n_u64(0),
            buffer: [0u8; 16],
    ...
    pub unsafe fn update_block(&mut self, block: &[u8; 16]) {
        let data = vld1q_u8(block.as_ptr());
        let data = byte_swap(data);
        let data = vreinterpretq_u64_u8(data);
    ...
    unsafe fn process_4_blocks(&mut self, data: &[u8]) {
        ...
        let m0 = vreinterpretq_u64_u8(byte_swap(vld1q_u8(data.as_ptr())));
        let m1 = vreinterpretq_u64_u8(byte_swap(vld1q_u8(data[16..].as_ptr())));
        let m2 = vreinterpretq_u64_u8(byte_swap(vld1q_u8(data[32..].as_ptr())));
        let m3 = vreinterpretq_u64_u8(byte_swap(vld1q_u8(data[48..].as_ptr())));
    ...
    pub unsafe fn finalize(mut self) -> [u8; 16] {
        ...
        let result = byte_swap(vreinterpretq_u8_u64(self.acc));
...
/// Convenience function to compute GHASH using NEON
///
/// # Safety
/// Requires NEON and AES/PMULL support
#[target_feature(enable = "neon", enable = "aes")]
pub unsafe fn ghash_neon(h: &[u8; 16], data: &[u8]) -> [u8; 16] {
    let mut hasher = GhashNeon::new(h);
    hasher.update(data);
    hasher.finalize()
}
...
mod tests {
    ...
    #[test]
    fn test_ghash_neon_empty() {
        let h = [0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b,
                 0x88, 0x4c, 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e];

        unsafe {
            let tag = ghash_neon(&h, &[]);
            assert_eq!(tag, [0u8; 16], "Empty input should produce zero tag");
        }
    }

    #[test]
    fn test_ghash_neon_deterministic() {
        let h = [0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b,
                 0x88, 0x4c, 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e];
        let data = [0x42u8; 64];

        unsafe {
            let tag1 = ghash_neon(&h, &data);
            let tag2 = ghash_neon(&h, &data);
            assert_eq!(tag1, tag2, "GHASH should be deterministic");
        }
    }

    #[test]
    fn test_ghash_neon_vs_software() {
    ...
    #[test]
    fn test_ghash_neon_partial_blocks() {
        use crate::ghash::ghash as ghash_software;
