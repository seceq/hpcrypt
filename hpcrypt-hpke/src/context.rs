...
mod tests {
    ...
    fn test_nonce_computation() {
        ...
        let mut ctx = HpkeContext {
            suite,
            key: vec![0u8; 16],
            base_nonce: vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B],
            sequence: 0,
        ...
        assert_eq!(nonce1, vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B]);

        // Increment sequence
        ctx.sequence = 1;
        let nonce2 = ctx.compute_nonce().unwrap();
        assert_eq!(nonce2, vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0A]);
    }
}
