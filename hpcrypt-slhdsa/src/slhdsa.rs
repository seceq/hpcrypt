    ...
    macro_rules! sign_with_stack_buffers {
        ($n:expr, $digest_size:expr) => {{
            ...
            let (fors_sig, _fors_pk, ht_sig) = with_hash!(P::N, P::HASH_TYPE, hash, {
                ...
                hash.prf_msg(secret_key.sk_prf(), &opt_rand_tmp[..P::N], message, opt_rand);

                // Hash message
                hash.h_msg(opt_rand, secret_key.pk_seed(), secret_key.pk_seed(), message, digest);
                ...
                let ht_sig = ht_sign::<P, _>(&fors_pk, secret_key.sk_seed(), secret_key.pk_seed(), tree_index, &mut addr, &hash);
    ...
    match P::N {
        ...
        _ => {
            ...
            let (fors_sig, _fors_pk, ht_sig) = with_hash!(P::N, P::HASH_TYPE, hash, {
                ...
                hash.h_msg(&opt_rand, secret_key.pk_seed(), secret_key.pk_seed(), message, &mut digest);
...
pub fn verify<P: ParameterSet>(public_key: &PublicKey<P>, message: &[u8], signature: &[u8]) -> bool {
    ...
    macro_rules! verify_with_stack_buffers {
        ($n:expr, $digest_size:expr) => {{
            ...
            with_hash!(P::N, P::HASH_TYPE, hash, {
                // Hash message
                hash.h_msg(opt_rand, public_key.pk_seed(), public_key.pk_root(), message, digest);
                ...
                ht_verify::<P, _>(fors_pk, ht_sig, public_key.pk_seed(), tree_index, public_key.pk_root(), &mut addr, &hash)
            })
        }};
    }
    ...
    match P::N {
        ...
        _ => {
            ...
            with_hash!(P::N, P::HASH_TYPE, hash, {
                hash.h_msg(opt_rand, public_key.pk_seed(), public_key.pk_root(), message, &mut digest);
