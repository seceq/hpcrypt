...
// Non-SIMD version - processes leaves sequentially
...
fn tree_hash_leaves_batch<P: ParameterSet, H: HashFunction>(
    ...
    // Process all leaves sequentially (non-SIMD version)
...
// =============================================================================
// XMSS Tree Functions (WOTS+ PK leaves for SLH-DSA hypertree)
// =============================================================================

use crate::wots::wots_pk_gen;

/// Compute an XMSS tree leaf, which is a WOTS+ public key.
///
/// In SLH-DSA/SPHINCS+, each layer's tree is an XMSS tree where leaves
/// are compressed WOTS+ public keys.
#[inline]
pub fn xmss_leaf<P: ParameterSet, H: HashFunction>(
    sk_seed: &[u8],
    pk_seed: &[u8],
    leaf_idx: usize,
    addr: &mut Address,
    ...
    // Set address for this WOTS+ key pair
    addr.set_keypair(leaf_idx as u32);

    // Generate WOTS+ public key (this is the leaf value)
    wots_pk_gen::<P, H>(sk_seed, pk_seed, addr, hash, output);
}

/// Compute XMSS tree root using treehash algorithm with WOTS+ PK leaves.
///
/// This computes the Merkle root of a tree where leaves are WOTS+ public keys.
pub fn xmss_treehash<P: ParameterSet, H: HashFunction>(
    sk_seed: &[u8],
    pk_seed: &[u8],
    leaf_idx_offset: usize,
    target_height: usize,
    ...
    // Use stack allocation for common N values
    match P::N {
        16 => {
            let mut stack: Vec<([u8; 16], usize)> = Vec::with_capacity(target_height + 1);

            for i in 0..n_leaves {
                // Generate XMSS leaf (WOTS+ pk)
                let mut node = [0u8; 16];
                xmss_leaf::<P, H>(sk_seed, pk_seed, leaf_idx_offset + i, addr, hash, &mut node);
                let mut node_height = 0usize;

                // Set address type for internal nodes and clear WOTS-specific fields
                addr.set_type(ADDR_TYPE_TREE);
...
/// Compute authentication path for an XMSS tree (WOTS+ PK leaves).
///
/// Returns the authentication path needed to verify a WOTS+ signature
/// at the given leaf position.
pub fn xmss_auth_path<P: ParameterSet, H: HashFunction>(
    sk_seed: &[u8],
    pk_seed: &[u8],
    leaf_idx: usize,
    tree_height: usize,
    ...
    for level in 0..tree_height {
        // Compute sibling subtree root at this level
        let sibling_idx = leaf_idx ^ (1 << level);

        let mut sibling = vec![0u8; P::N];

        if level == 0 {
            // Sibling is a leaf (WOTS+ pk)
            xmss_leaf::<P, H>(sk_seed, pk_seed, sibling_idx, addr, hash, &mut sibling);
        } else {
            // Sibling is a subtree root
            let sibling_subtree_start = (sibling_idx >> level) << level;
            xmss_treehash::<P, H>(sk_seed, pk_seed, sibling_subtree_start, level, addr, hash, &mut sibling);
        }
