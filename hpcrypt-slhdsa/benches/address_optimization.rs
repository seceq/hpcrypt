...
fn to_bytes_unsafe_ptr(addr: &Address) -> [u8; 32] {
    let words = unsafe { core::slice::from_raw_parts(addr as *const Address as *const u32, 8) };

    let mut bytes = [0u8; 32];
    for (i, &word) in words.iter().enumerate() {
        let word_bytes = word.to_be_bytes();
        unsafe {
            core::ptr::copy_nonoverlapping(word_bytes.as_ptr(), bytes.as_mut_ptr().add(i * 4), 4);
        }
    }
    bytes
}
...
fn bench_to_bytes_single_call(c: &mut Criterion) {
    ...
    group.bench_function("baseline", |b| {
        b.iter(|| black_box(to_bytes_baseline(black_box(&addr))))
    });

    group.bench_function("unsafe_ptr", |b| {
        b.iter(|| black_box(to_bytes_unsafe_ptr(black_box(&addr))))
    });

    group.bench_function("transmute", |b| {
        b.iter(|| black_box(to_bytes_transmute(black_box(&addr))))
    });

    group.bench_function("macro_unrolled", |b| {
        b.iter(|| black_box(to_bytes_macro_unrolled(black_box(&addr))))
    });

    group.bench_function("array_ops", |b| {
        b.iter(|| black_box(to_bytes_array_ops(black_box(&addr))))
    });
