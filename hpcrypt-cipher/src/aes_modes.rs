...
impl AesCbc128 {
    ...
    pub fn encrypt(&self, iv: &[u8; IV_SIZE], plaintext: &[u8]) -> Result<Vec<u8>, CipherError> {
        if plaintext.len() % BLOCK_SIZE != 0 {
            return Err(CipherError::InvalidPlaintextLength { block_size: BLOCK_SIZE, actual: plaintext.len() });
        }
    ...
    pub fn decrypt(&self, iv: &[u8; IV_SIZE], ciphertext: &[u8]) -> Result<Vec<u8>, CipherError> {
        if ciphertext.len() % BLOCK_SIZE != 0 {
            return Err(CipherError::InvalidPlaintextLength { block_size: BLOCK_SIZE, actual: ciphertext.len() });
        }
...
impl AesXts128 {
    ...
    pub fn encrypt(&self, tweak: &[u8; 16], plaintext: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        // Process full blocks
        let blocks_to_process = if has_partial { full_blocks - 1 } else { full_blocks };
        ...
        if has_partial {
            ...
            let block: [u8; BLOCK_SIZE] = plaintext[last_full_block_idx * BLOCK_SIZE..(last_full_block_idx + 1) * BLOCK_SIZE]
                .try_into()
                .unwrap();
    ...
    pub fn decrypt(&self, tweak: &[u8; 16], ciphertext: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        if has_partial {
            ...
            let cp: [u8; BLOCK_SIZE] = ciphertext[cp_idx..cp_idx + BLOCK_SIZE]
                .try_into()
                .unwrap();
...
mod tests {
    ...
    fn test_gf128_mul_x() {
        ...
        let expected = [0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        ...
        let input = [0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        let expected = [0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        assert_eq!(gf128_mul_x(&input), expected);
        ...
        let expected = [0x87, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        assert_eq!(gf128_mul_x(&input), expected);
    }
