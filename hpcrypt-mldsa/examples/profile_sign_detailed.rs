...
fn main() {
    ...
    println!("Throughput:    {:.1} signs/sec\n", 1_000_000.0 / avg_micros as f64);
    ...
    let per_attempt_est = sample_y_est + early_check_est + matrix_mul_est +
                         high_bits_est + challenge_hash_est + sample_c_est +
                         compute_z_est + check_z_est + compute_r_est +
                         check_r0_est + hints_est + check_hints_est;
    ...
    println!("  ρ' hash (H(K || rnd || μ)):    {:>6.1} µs", rho_prime_hash_est);
    println!("  Expand matrix A:                {:>6.1} µs", expand_a_est);
    println!("  Subtotal:                       {:>6.1} µs ({:.1}%)",
        setup_est, 100.0 * setup_est / total_us);
    println!();

    println!("Per-attempt ({:.1} avg attempts):", avg_attempts);
    println!("  Sample y (L={} polys):         {:>6.1} µs", 5, sample_y_est);
    println!("  Early rejection check:          {:>6.1} µs", early_check_est);
    println!("  Matrix multiply (A·y):         {:>6.1} µs", matrix_mul_est);
    println!("  Extract high bits (w1):         {:>6.1} µs", high_bits_est);
    println!("  Challenge hash (H(μ || w1)):   {:>6.1} µs", challenge_hash_est);
    ...
    println!("  Check hint count:               {:>6.1} µs", check_hints_est);
    println!("  Subtotal per attempt:           {:>6.1} µs", per_attempt_est);
    println!("  × {:.1} attempts:               {:>6.1} µs ({:.1}%)",
        avg_attempts, per_attempt_est * avg_attempts,
    ...
    let mut bottlenecks = vec![
        ...
        ("Matrix multiply A·y", matrix_mul_est * avg_attempts, per_attempt_est * avg_attempts),
        ("Compute r (w - c·s2)", compute_r_est * avg_attempts, per_attempt_est * avg_attempts),
        ("Sample y", sample_y_est * avg_attempts, per_attempt_est * avg_attempts),
        ("Make hints", hints_est * avg_attempts, per_attempt_est * avg_attempts),
        ("Compute z (y + c·s1)", compute_z_est * avg_attempts, per_attempt_est * avg_attempts),
    ];
    ...
    for (i, (name, time, _)) in bottlenecks.iter().enumerate() {
        println!("  {:2}  | {:<26} | {:>6.1} µs | {:>6.1}%",
            i+1, name, time, 100.0 * time / total_us);
    }
    println!();
    ...
    println!();
    println!("1. Matrix multiply A·y ({:.0} µs, {:.1}%)",
    ...
    println!("   - Potential gain: 10-15% → ~{:.0} µs", 0.125 * matrix_mul_est * avg_attempts);
    println!();

    println!("2. Expand matrix A ({:.0} µs, {:.1}%)",
        expand_a_est,
    ...
    println!("3. Compute r (w - c·s2) ({:.0} µs, {:.1}%)",
    ...
    println!("   - Potential gain: 20% → ~{:.0} µs", 0.2 * compute_r_est * avg_attempts);
    println!();

    println!("4. Sample y ({:.0} µs, {:.1}%)",
    ...
    println!("   - Potential gain: 10% → ~{:.0} µs", 0.1 * sample_y_est * avg_attempts);
    println!();
