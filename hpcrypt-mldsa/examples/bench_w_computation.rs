...
fn main() {
    ...
    println!("  Modular exponentiation: {:.1}x slower", ns_per_compute as f64 / ns_per_lookup as f64);
    println!("  Iterative multiplication: {:.1}x slower", ns_per_iter as f64 / ns_per_lookup as f64);
    ...
    println!("  Modular exp overhead: {} ns ({:.2} µs)", ntt_overhead_exp, ntt_overhead_exp as f64 / 1000.0);
    println!("  Iterative overhead: {} ns ({:.2} µs)", ntt_overhead_iter, ntt_overhead_iter as f64 / 1000.0);
    ...
    println!("  Modular exp overhead: {:.2} µs", sign_overhead_exp as f64 / 1000.0);
    println!("  Iterative overhead: {:.2} µs", sign_overhead_iter as f64 / 1000.0);
    ...
    if ns_per_compute > ns_per_lookup * 10 {
        ...
        println!("⚠️  On-the-fly computation is 2-10x slower - questionable");
    } else {
    ...
    println!("  Cost: {:.1}x slower performance", ns_per_compute as f64 / ns_per_lookup as f64);
}
