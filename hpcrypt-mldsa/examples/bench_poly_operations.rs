...
fn count_operations_in_signing() {
    ...
    println!("     - Matrix-vector multiply: {} × {} polynomial multiplications", k, l);
    ...
    println!("  - Total Barrett reductions: {} × 256 = {} reductions",
    ...
    println!("  - poly.add(): {:.0} calls", adds_per_attempt as f64 * avg_attempts);
    println!("  - poly.sub(): {:.0} calls", subs_per_attempt as f64 * avg_attempts);
    println!("  - Total Barrett reductions: {:.0}",
...
fn main() {
    ...
    println!("poly.add():    {:.2} ns/op  ({} barrett_reduce calls)", add_time, N);
    
    let sub_time = benchmark_poly_sub(iterations);
    println!("poly.sub():    {:.2} ns/op  ({} barrett_reduce calls)", sub_time, N);
    
    let reduce_time = benchmark_poly_reduce(iterations);
    println!("poly.reduce(): {:.2} ns/op  ({} barrett_reduce calls)", reduce_time, N);
    println!();
    
    let barrett_time_per_coeff = add_time / N as f64;
    println!("Per-coefficient Barrett reduction: {:.3} ns", barrett_time_per_coeff);
    println!();
    ...
    println!("Barrett reductions per signature: {:.0}", barrett_ops_per_sig);
    println!("Time in Barrett per signature: {:.2} µs", barrett_time_per_sig_us);
    println!();
    ...
    println!("Barrett+Shoup improvement: {:.1}% faster", shoup_improvement * 100.0);
    println!("New Barrett time: {:.2} µs (was {:.2} µs)", new_barrett_time, barrett_time_per_sig_us);
    ...
    if overall_improvement < 1.0 {
        ...
        println!();
        println!("Reasons:");
        println!("  1. Barrett operations are only {:.1}% of signing time", barrett_percentage);
        println!("  2. Even with 11.7% Barrett speedup → only {:.2}% overall gain", overall_improvement);
        ...
        println!("⚠️  MARGINAL - Barrett+Shoup would provide {:.2}% overall improvement", overall_improvement);
        println!();
        ...
        println!("✅ WORTH IT - Barrett+Shoup would provide {:.2}% overall improvement", overall_improvement);
        println!();
