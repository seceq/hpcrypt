...
//!
//! Tests for X25519 key exchange from RFC 7748 Sections 5.2 and 6.1.
//!
//! X25519 provides:
//! - Elliptic Curve Diffie-Hellman (ECDH) key agreement
//! - 32-byte (256-bit) keys
//! - 32-byte (256-bit) shared secrets
...
fn test_x25519_rfc7748() {
    let test_vectors: Vec<X25519TestVector> = load_test_file("rfc7748-x25519.json");

    println!("\n=== RFC 7748: X25519 Key Exchange Tests ===");
...
fn test_scalar_mult(test: &X25519TestVector, stats: &mut TestStats) {
    ...
    if scalar_bytes.len() != 32 {
        println!(
            "  Test {} SKIPPED: Invalid scalar size {} (expected 32)",
            test.test_id,
            scalar_bytes.len()
        );
        stats.skipped += 1;
    ...
    if u_bytes.len() != 32 {
        println!(
            "  Test {} SKIPPED: Invalid u-coordinate size {} (expected 32)",
            test.test_id,
            u_bytes.len()
        );
        stats.skipped += 1;
    ...
    if expected_output.len() != 32 {
        println!(
            "  Test {} SKIPPED: Invalid expected output size {} (expected 32)",
            test.test_id,
            expected_output.len()
        );
        stats.skipped += 1;
    ...
    let u: [u8; 32] = u_bytes.try_into().unwrap();
    let expected: [u8; 32] = expected_output.try_into().unwrap();

    // Perform scalar multiplication
    let result = match X25519::shared_secret(&scalar, &u) {
        Ok(r) => r,
        Err(e) => {
            println!(
                "  Test {} FAILED: X25519 computation error: {:?}",
                test.test_id, e
            );
            stats.failed += 1;
            return;
...
fn test_iterative(test: &X25519TestVector, stats: &mut TestStats) {
    ...
    if scalar_bytes.len() != 32 || u_bytes.len() != 32 || expected_output.len() != 32 {
        println!("  Test {} SKIPPED: Invalid input sizes", test.test_id);
        stats.skipped += 1;
        return;
    }

    let mut k: [u8; 32] = scalar_bytes.try_into().unwrap();
    let mut u: [u8; 32] = u_bytes.try_into().unwrap();
    let expected: [u8; 32] = expected_output.try_into().unwrap();
    ...
    for i in 0..test.iterations {
        ...
        let result = match X25519::shared_secret(&k, &u) {
            Ok(r) => r,
            Err(e) => {
                println!(
                    "  Test {} FAILED: X25519 computation error at iteration {}: {:?}",
                    test.test_id, i, e
                );
                stats.failed += 1;
                return;
...
fn test_diffie_hellman(test: &X25519TestVector, stats: &mut TestStats) {
    ...
    if alice_private_bytes.len() != 32
        || alice_public_bytes.len() != 32
        || bob_private_bytes.len() != 32
        || bob_public_bytes.len() != 32
        || expected_shared_secret.len() != 32
    {
    ...
    let alice_public_expected: [u8; 32] = alice_public_bytes.try_into().unwrap();
    let bob_private: [u8; 32] = bob_private_bytes.try_into().unwrap();
    let bob_public_expected: [u8; 32] = bob_public_bytes.try_into().unwrap();
    let expected_secret: [u8; 32] = expected_shared_secret.try_into().unwrap();

    // Test 1: Verify Alice's public key generation
    println!("  Testing Alice's public key generation...");
    let alice_public_computed = X25519::public_key(&alice_private);
    ...
    let bob_public_computed = X25519::public_key(&bob_private);
    ...
    let alice_shared_secret = match X25519::shared_secret(&alice_private, &bob_public_expected) {
        Ok(s) => s,
        Err(e) => {
            println!(
    ...
    let bob_shared_secret = match X25519::shared_secret(&bob_private, &alice_public_expected) {
        Ok(s) => s,
        Err(e) => {
            println!(
