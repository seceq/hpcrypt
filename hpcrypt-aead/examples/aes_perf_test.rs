...
fn main() {
    ...
    let improvement_pct = ((baseline_time as f64 - optimized_time as f64) / baseline_time as f64) * 100.0;

    println!("Speedup:    {:.3}x", speedup);
    println!("Improvement: {:.1}%", improvement_pct);

    if improvement_pct >= 20.0 {
        println!("\n✅ SUCCESS: Achieved {}% improvement (target: 20-45%)", improvement_pct as i32);
    } else if improvement_pct >= 15.0 {
        println!("\n⚠️  PARTIAL: Achieved {}% improvement (target: 20-45%)", improvement_pct as i32);
    } else {
        println!("\n❌ BELOW TARGET: Only {}% improvement (target: 20-45%)", improvement_pct as i32);
    }
}
