...
fn test_isolated_sbox_single_byte() {
    ...
    assert_eq!(
        result_blocks[0][0], 0xd4,
        "S-box should transform 0x19 -> 0xd4"
    );
}
...
fn test_compare_with_rustcrypto_aes() {
    ...
    println!(
        "NIST expected output: [39, 25, 84, 1d, 02, dc, 09, fb, dc, 11, 85, 97, 19, 6a, 0b, 32]"
    );
    ...
    println!(
        "All 4 blocks should be identical to: {:02x?}",
        rust_crypto_output
    );
    ...
    println!(
        "First block after SubBytes: {:02x?}",
        first_block_after_subbytes
    );
    ...
    if first_block_after_subbytes == expected_after_subbytes {
        ...
        for i in 0..16 {
            println!(
                "  Byte {}: got 0x{:02x}, expected 0x{:02x}",
...
fn test_compare_bitslicing_with_rustcrypto() {
    ...
    for i in 0..8 {
        ...
        println!(
            "  state[{}] (bit {}): lowest 16 bits = 0x{:04x} {}",
            i,
            7 - i,
            lowest_bits,
            if val == 0 { "" } else { "✓" }
...
fn test_nist_aes128_encrypt() {
    ...
    assert_eq!(
        ...
        "AES-128 encryption should match NIST test vector"
    );
}
...
fn test_nist_aes128_encrypt_c1() {
    ...
    assert_eq!(
        ciphertext, expected_ciphertext,
        "AES-128 encryption should match NIST test vector C.1"
    );
}
...
fn test_nist_aes128_decrypt() {
    ...
    assert_eq!(
        ...
        "AES-128 decryption should match NIST test vector"
    );
}
...
fn test_different_blocks_in_parallel() {
    ...
    assert_eq!(
        blocks, originals,
        "Encrypt-decrypt roundtrip should work for 4 different blocks"
    );
}
...
fn test_edge_case_all_zeros() {
    ...
    assert_ne!(
        ...
        "All zeros should not encrypt to all zeros"
    );
}
...
fn test_edge_case_alternating_bits() {
    ...
    assert_eq!(
        plaintext, decrypted,
        "Alternating bits roundtrip should work"
    );
}
...
fn test_edge_case_single_bit() {
    ...
    for bit_pos in 0..128 {
        ...
        assert_eq!(
            plaintext, decrypted,
            "Single bit at position {} roundtrip should work",
            bit_pos
        );
    }
}
...
fn test_parallel_consistency() {
    ...
    assert_eq!(
        blocks[0], blocks[1],
        "Parallel encryption should produce identical results"
    );
    assert_eq!(
        blocks[1], blocks[2],
        "Parallel encryption should produce identical results"
...
fn test_block_independence() {
    ...
    assert_eq!(
        cipher1_solo, blocks_together[0],
        "Block 1 should match solo encryption"
    );
    assert_eq!(
        cipher2_solo, blocks_together[1],
        "Block 2 should match solo encryption"
    );
    assert_eq!(
        cipher1_solo, blocks_together[2],
        "Block 1 duplicate should match"
    );
    assert_eq!(
        cipher2_solo, blocks_together[3],
        "Block 2 duplicate should match"
    );
}
