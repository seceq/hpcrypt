...
//! ChaCha20-Poly1305 AEAD implementations against known edge cases and potential vulnerabilities.
//!
//! References:
//! - https://github.com/google/wycheproof
//! - RFC 7539 (ChaCha20-Poly1305)
//! - RFC 8439 (Updated ChaCha20-Poly1305)
...
fn run_chacha20poly1305_test(test: &TestCase) -> bool {
    // Check for valid key size (256 bits = 32 bytes)
    if test.key.len() != 32 {
        match test.result {
            TestResult::Valid => {
                eprintln!(
                    "ChaCha20-Poly1305 Test {} FAILED: Valid test has invalid key size: {}",
    ...
    // Check for valid nonce size (96 bits = 12 bytes)
    if test.iv.len() != 12 {
        match test.result {
            TestResult::Valid => {
                eprintln!(
                    "ChaCha20-Poly1305 Test {} FAILED: Valid test has invalid nonce size: {}",
    ...
    if test.tag.len() != 16 {
        match test.result {
            TestResult::Valid => {
                eprintln!(
                    "ChaCha20-Poly1305 Test {} FAILED: Valid test has invalid tag size: {}",
    ...
    match test.result {
        TestResult::Valid => {
            ...
            let ciphertext_with_tag = ChaCha20Poly1305::encrypt(&key, &nonce, &test.msg, &test.aad);
            ...
            if !constant_time_compare(&ciphertext_with_tag, &expected) {
                eprintln!(
                    "ChaCha20-Poly1305 Test {} FAILED: Valid test encryption mismatch: {}",
            ...
            let decrypted = ChaCha20Poly1305::decrypt(&key, &nonce, &ciphertext_with_tag, &test.aad);

            match decrypted {
                Some(plaintext) => {
                    if !constant_time_compare(&plaintext, &test.msg) {
                        eprintln!(
                            "ChaCha20-Poly1305 Test {} FAILED: Valid test decryption mismatch: {}",
                            test.tc_id, test.comment
                        );
                        return false;
                    }
                ...
                None => {
                    eprintln!(
                        "ChaCha20-Poly1305 Test {} FAILED: Valid ciphertext rejected: {}",
                        test.tc_id, test.comment
                    );
                    return false;
                }
        ...
        TestResult::Invalid => {
            ...
            if decrypted.is_some() {
                eprintln!(
                    "ChaCha20-Poly1305 Test {} FAILED: Invalid ciphertext accepted: {}",
                    test.tc_id, test.comment
                );
...
#[test]
fn wycheproof_chacha20poly1305() {
    ...
    for group in test_file.test_groups {
        ...
        for test in group.tests {
            ...
            if run_chacha20poly1305_test(&test) {
                passed_tests += 1;
            }
        }
    }
    ...
    assert_eq!(
        passed_tests,
        total_tests,
        "Some ChaCha20-Poly1305 tests failed (note: {} tests were skipped for unsupported parameters)",
        skipped_tests
    );
}
...
// Test edge cases
#[test]
fn chacha20poly1305_edge_cases() {
    let key = [0u8; 32];
    let nonce = [0u8; 12];

    // Test with empty message
    let ciphertext1 = ChaCha20Poly1305::encrypt(&key, &nonce, b"", b"");
    assert_eq!(ciphertext1.len(), 16, "Empty message should produce 16-byte tag only");

    let decrypted1 = ChaCha20Poly1305::decrypt(&key, &nonce, &ciphertext1, b"");
    assert_eq!(decrypted1, Some(vec![]), "Decryption of empty message should succeed");

    // Test with non-empty message
    let msg = b"Hello, ChaCha20-Poly1305!";
    let ciphertext2 = ChaCha20Poly1305::encrypt(&key, &nonce, msg, b"");
    assert_eq!(ciphertext2.len(), msg.len() + 16, "Ciphertext should be plaintext + 16-byte tag");

    let decrypted2 = ChaCha20Poly1305::decrypt(&key, &nonce, &ciphertext2, b"");
    assert_eq!(decrypted2, Some(msg.to_vec()), "Decryption should recover plaintext");

    // Test with AAD
    let aad = b"additional authenticated data";
    let ciphertext3 = ChaCha20Poly1305::encrypt(&key, &nonce, msg, aad);

    let decrypted3 = ChaCha20Poly1305::decrypt(&key, &nonce, &ciphertext3, aad);
    assert_eq!(decrypted3, Some(msg.to_vec()), "Decryption with AAD should succeed");

    // Wrong AAD should fail
    let decrypted4 = ChaCha20Poly1305::decrypt(&key, &nonce, &ciphertext3, b"wrong aad");
    ...
    modified[0] ^= 1;
    let decrypted5 = ChaCha20Poly1305::decrypt(&key, &nonce, &modified, b"");
    ...
    let decrypted6 = ChaCha20Poly1305::decrypt(&key, &nonce, &modified_tag, b"");
    assert_eq!(decrypted6, None, "Decryption with modified tag should fail");

    println!("ChaCha20-Poly1305 edge case tests: All passed");
}
