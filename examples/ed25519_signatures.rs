...
fn main() {
    ...
    println!("   Private key: {:02x}{:02x}{:02x}{:02x}...{:02x}{:02x}{:02x}{:02x}",
             private_key[0], private_key[1], private_key[2], private_key[3],
             private_key[28], private_key[29], private_key[30], private_key[31]);
    println!("   Public key:  {:02x}{:02x}{:02x}{:02x}...{:02x}{:02x}{:02x}{:02x}",
    ...
    println!("   Signature:   {:02x}{:02x}{:02x}{:02x}...{:02x}{:02x}{:02x}{:02x} ({} bytes)",
    ...
    println!("   Original message: {:?}", std::str::from_utf8(original_message).unwrap());
    ...
    println!("   Tampered message: {:?}", std::str::from_utf8(tampered_message).unwrap());
    println!("   Signature valid:  {}",
    ...
    println!("   Message:           {:?}", std::str::from_utf8(message3).unwrap());
    println!("   Correct key verification: {}",
             if Ed25519::verify(&public_key, message3, &signature3) {
                 "✓ Valid"
             } else {
                 "✗ Invalid"
             });
    println!("   Wrong key verification:   {}",
    ...
    println!("   Can verify:   {}", if empty_valid { "✓ Yes" } else { "✗ No" });
    println!();
    ...
    println!("   Signature:    {} bytes (always 64)", large_signature.len());
    println!("   Verified:     {}", if large_valid { "✓ Yes" } else { "✗ No" });
    println!();
    ...
    println!("   Signature:    {:02x}{:02x}{:02x}{:02x}...{:02x}{:02x}{:02x}{:02x}",
             rfc_signature[0], rfc_signature[1], rfc_signature[2], rfc_signature[3],
             rfc_signature[60], rfc_signature[61], rfc_signature[62], rfc_signature[63]);
    println!("   RFC Compliant: {}", if rfc_valid { "✓ Yes" } else { "✗ No" });
