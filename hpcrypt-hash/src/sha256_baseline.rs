...
impl Sha256 {
    ...
    fn process_block(&mut self) {
        // Circular buffer optimization: use only 16 entries instead of 64
        // This reduces memory footprint from 256 bytes to 64 bytes
        ...
        for i in 0..64 {
            // Update message schedule for rounds 16-63
            // Optimization: cache array lookups to reduce redundant memory access
            if i >= 16 {
            ...
            let ch = g ^ (e & (f ^ g)); // Optimized Ch function: 3 ops instead of 5
            let temp1 = h
                .wrapping_add(s1)
                .wrapping_add(ch)
                .wrapping_add(K[i])
            ...
            let maj = (a & b) | (c & (a | b)); // Optimized Maj function: 4 ops instead of 5
