//! SHA-512/224 (Secure Hash Algorithm 512/224)
//!
//! SHA-512/224 is a variant of SHA-512 with different initial values
...
/// SHA-512/224 block size in bytes (same as SHA-512: 1024 bits = 128 bytes)
pub const BLOCK_LEN: usize = 128;

/// SHA-512/224 output size in bytes (224 bits = 28 bytes)
...
impl Sha512_224 {
    ...
    fn process_block(&mut self) {
        ...
        // Remaining 64 rounds in groups of 4
        update_w!(w, 16);
...
impl crate::traits::HashFunction for Sha512_224 {
    ...
    fn new() -> Self {
        Self {
            // SHA-512/224 initial values (specified in FIPS 180-4)
            h: [
                0x8c3d37c819544da2,
                0x73e1996689dcd4d6,
                0x1dfab7ae32ff9c82,
    ...
    fn finalize(mut self) -> Self::Output {
        ...
        // Output first 28 bytes (3.5 words) of state
        let mut out = [0u8; OUTPUT_LEN];
        for i in 0..3 {
            write_u64_be(&mut out[i * 8..(i + 1) * 8], self.h[i]);
        }
        // Last 4 bytes from the 4th word
...
pub fn sha512_224(data: &[u8]) -> [u8; OUTPUT_LEN] {
    use crate::traits::HashFunction;
    Sha512_224::hash(data)
}

// Helper function to write u32 in big-endian
#[inline]
fn write_u32_be(dest: &mut [u8], val: u32) {
    dest[0] = (val >> 24) as u8;
    dest[1] = (val >> 16) as u8;
    dest[2] = (val >> 8) as u8;
    dest[3] = val as u8;
