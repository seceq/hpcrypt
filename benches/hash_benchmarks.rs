...
fn bench_hashes_small(c: &mut Criterion) {
    ...
    group.bench_function("blake3", |b| b.iter(|| blake3(black_box(&data))));

    group.bench_function("blake2b", |b| b.iter(|| blake2b(black_box(&data))));

    group.bench_function("sha256", |b| b.iter(|| sha256(black_box(&data))));
...
fn bench_incremental_vs_oneshot(c: &mut Criterion) {
    ...
    group.bench_function("one_shot", |b| b.iter(|| blake3(black_box(&data))));
