...
use hpcrypt_curves::p521::{FieldElement, field_montgomery_native::MontgomeryFieldElement};
...
fn bench_single_multiplication(c: &mut Criterion) {
    ...
    let a_native = FieldElement::from_limbs([
        ...
        0x9999aaaabbbbcccc, 0xddddeeeeffffaaaa,
        0xbbbbccccddddeeee, 0xffff111122223333,
        0x00000000000001ff,
    ]);
    let b_native = FieldElement::from_limbs([
        0xabcdef1234567890, 0x0123456789abcdef,
        0x4444333322221111, 0x8888777766665555,
        0xccccbbbbaaaa9999, 0xaaaaffffeeeedddd,
        0xeeeeddddccccbbbb, 0x3333222211110000,
        0x00000000000001aa,
    ]);

    group.bench_function("Native Mersenne", |b| {
        b.iter(|| {
            black_box(a_native.mul(&b_native))
        });
    });
...
fn bench_batch_multiplication(c: &mut Criterion) {
    ...
    for count in [10, 20, 50].iter() {
        group.bench_with_input(BenchmarkId::new("Native Mersenne", count), count, |bench, &count| {
            bench.iter(|| {
...
fn bench_addition(c: &mut Criterion) {
    ...
    group.bench_function("Native Mersenne", |b| {
        b.iter(|| {
            black_box(a_native.add(&b_native))
        });
    });
...
fn bench_conversion(c: &mut Criterion) {
    ...
    group.bench_function("Native from_bytes", |b| {
        b.iter(|| {
            black_box(FieldElement::from_bytes(&bytes_native))
        });
    });
    ...
    group.bench_function("Montgomery to_bytes", |b| {
        b.iter(|| {
            black_box(a_montgomery.to_bytes())
        });
    });

    group.bench_function("Montgomery from_bytes", |b| {
        b.iter(|| {
            black_box(MontgomeryFieldElement::from_bytes(&bytes_mont))
        });
    });
