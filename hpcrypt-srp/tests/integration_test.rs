...
use hpcrypt_srp::{register_user, register_user_with_hash, SrpClient, SrpGroup, SrpHashFunction, SrpServer};
use rand::thread_rng;
...
fn test_full_authentication_flow() {
    ...
    let registration = register_user(b"alice", b"password123", SrpGroup::Srp2048, &mut rng).unwrap();
...
fn test_wrong_password_fails() {
    ...
    let registration = register_user(b"alice", b"correct_password", SrpGroup::Srp2048, &mut rng).unwrap();
...
fn test_zero_public_key_rejected() {
    ...
    let mut server = SrpServer::new(&registration.verifier, &registration.salt, b"alice", SrpGroup::Srp2048);
...
fn test_deterministic_verifier() {
    ...
    let v1 = hpcrypt_srp::create_verifier(b"alice", b"password123", &salt, SrpGroup::Srp2048).unwrap();
    let v2 = hpcrypt_srp::create_verifier(b"alice", b"password123", &salt, SrpGroup::Srp2048).unwrap();

    assert_eq!(v1, v2);

    // Different password should produce different verifier
    let v3 = hpcrypt_srp::create_verifier(b"alice", b"different", &salt, SrpGroup::Srp2048).unwrap();
    assert_ne!(v1, v3);

    // Different username should produce different verifier
    let v4 = hpcrypt_srp::create_verifier(b"bob", b"password123", &salt, SrpGroup::Srp2048).unwrap();
    assert_ne!(v1, v4);
}
...
fn test_authentication_with_different_hash_functions() {
    ...
    // Test SHA-256 (default)
    {
        ...
        client.process_server_response(&b_pub, server.get_salt()).unwrap();
        let m1 = client.compute_proof().unwrap();
