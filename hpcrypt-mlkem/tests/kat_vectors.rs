...
fn kat_mlkem512_vector_1() {
    ...
    assert_eq!(
        result.1, ss_recovered,
        "Decapsulated shared secret must match"
    );
}
...
fn kat_all_parameter_sets_unique_outputs() {
    ...
    assert_ne!(
        kp512.encapsulation_key().len(),
        kp768.encapsulation_key().len()
    );
    assert_ne!(
        kp768.encapsulation_key().len(),
        kp1024.encapsulation_key().len()
    );
...
fn kat_sequential_seeds() {
    ...
    for i in 0u8..10 {
        ...
        assert_eq!(ss1, ss2, "Roundtrip must work for seed with value {}", i);
    }
}
