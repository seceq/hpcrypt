...
fn test_10g_multiple_methods_match() {
    ...
    let expected_x_bytes: [u8; 48] = [
        0xa6, 0x69, 0xc5, 0x56, 0x3b, 0xd6, 0x7e, 0xec, 0x67, 0x8d, 0x29, 0xd6,
        0xef, 0x4f, 0xde, 0x86, 0x4f, 0x37, 0x2d, 0x90, 0xb7, 0x9b, 0x9e, 0x88,
        0x93, 0x1d, 0x5c, 0x29, 0x29, 0x12, 0x38, 0xcc, 0xed, 0x8e, 0x85, 0xab,
        0x50, 0x7b, 0xf9, 0x1a, 0xa9, 0xcb, 0x2d, 0x13, 0x18, 0x66, 0x58, 0xfb
    ];

    // After fix, all methods should produce the same (correct) result
    assert_eq!(aff1.x.to_bytes().as_ref(), &expected_x_bytes,
        "Method 1 (9*G+G) should match Python reference");
    assert_eq!(aff2.x.to_bytes().as_ref(), &expected_x_bytes,
        "Method 2 (ladder) should match Python reference");
    assert_eq!(aff3.x.to_bytes().as_ref(), &expected_x_bytes,
        "Method 3 (8*G+2*G) should match Python reference");
    assert_eq!(aff4.x.to_bytes().as_ref(), &expected_x_bytes,
...
fn test_all_failing_k_values() {
    ...
    let failing_k_values = [
        10, 19, 24, 25, 28, 32, 36, 38, 39, 64, 76, 80, 88, 90, 102, 119,
        120, 125, 130, 131, 145, 146, 148, 159, 169, 170, 179, 192, 193,
        204, 209, 212, 223, 225, 227, 232, 233, 243, 248, 249, 252, 254, 255
    ];
    ...
    for &k in &failing_k_values {
        ...
        assert_eq!(aff_ladder.x, aff_add.x,
            "k={}: ladder and repeated addition should match (x-coordinate)", k);
        assert_eq!(aff_ladder.y, aff_add.y,
            "k={}: ladder and repeated addition should match (y-coordinate)", k);
    }
}
