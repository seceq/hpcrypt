...
pub fn batch_invert(elems: &mut [FieldElement]) {
    ...
    for i in 1..n {
        let prod = products[i - 1].mul(&elems[i]);
        // Check if product looks unreduced (has high bits set in upper limbs)
        // In a properly reduced secp256k1 element, limbs should be less than modulus
        products.push(prod);
    }
