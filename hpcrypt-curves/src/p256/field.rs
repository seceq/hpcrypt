...
//! P-256 prime modulus. All operations are constant-time to prevent timing
//! attacks.
//!
//! # Representation
//!
//! Field elements are represented using 4 x 64-bit limbs in little-endian order.
...
impl FieldElement {
    ...
    /// Creates a field element from 4 limbs (little-endian).
    ///
    /// # Note
    ///
    ...
    fn is_canonical(&self) -> bool {
        ...
        for i in (0..4).rev() {
            if self.limbs[i] < P256_MODULUS[i] {
                return true;
            }
            if self.limbs[i] > P256_MODULUS[i] {
                return false;
...
impl fmt::Debug for FieldElement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "FieldElement({:016x}_{:016x}_{:016x}_{:016x})",
...
mod tests {
    ...
    #[test]
    fn test_from_limbs() {
        let fe = FieldElement::from_limbs([0x1234, 0x5678, 0x9ABC, 0xDEF0]);
    ...
    fn test_from_bytes_rejects_modulus() {
        // P-256 modulus should be rejected (not canonical)
        let modulus = FieldElement::from_limbs(P256_MODULUS);
    ...
    fn test_from_bytes_accepts_one() {
        let mut bytes = [0u8; 32];
        bytes[31] = 1; // Little-endian in memory, but big-endian encoding
        let fe = FieldElement::from_bytes(&bytes).unwrap();
