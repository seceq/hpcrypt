...
///
/// Trade-offs:
/// - w=4: 8 odd multiples, ~512 bytes, ~35% speedup (DEFAULT)
/// - w=5: 16 odd multiples, ~1 KB, ~45% speedup (FAST)
/// - w=6: 32 odd multiples, ~2 KB, ~50% speedup (VERY_FAST)
///
/// Use feature flags to select:
/// - Default: w=4 (balanced)
/// - `scalar-mul-fast`: w=5 (recommended for most use cases)
/// - `scalar-mul-very-fast`: w=6 (maximum performance, more memory)
...
/// - w=6: values in range [-63, 63], 32 odd multiples
...
/// Perform scalar multiplication using wNAF with Montgomery arithmetic
///
/// This is an optimized version that uses Montgomery-form arithmetic for all
/// point operations, providing significant speedup:
/// - 45% faster point doubling
/// - 22% faster point addition
/// - Overall ~24% faster scalar multiplication
///
/// # Arguments
///
/// * `point` - The base point P
...
/// # Algorithm
///
/// Same as `wnaf_scalar_mul` but uses `PointMontgomery` for all operations:
/// 1. Convert point to Montgomery form once at start
/// 2. Compute wNAF representation
/// 3. Precompute odd multiples (in Montgomery form)
/// 4. Perform scalar multiplication using Montgomery operations
/// 5. Convert result back to standard form once at end
///
/// # Performance
///
/// Benchmarks show ~24% speedup compared to standard wNAF:
/// - Pure Montgomery doubling: 45% faster (159ns vs 231ns)
/// - Pure Montgomery addition: 22% faster (506ns vs 616ns)
/// - Conversion overhead: negligible (59ns total)
pub fn wnaf_scalar_mul_montgomery(point: &Point, scalar: &[u8; 32], width: usize) -> Point {
    use super::PointMontgomery;

    // Convert input point to Montgomery form once
    let point_mont = PointMontgomery::from_point(point);

    // Compute wNAF representation (same as standard)
    let wnaf = compute_wnaf(scalar, width);

    // Precompute odd multiples in Montgomery form
    let table = WNafTableMontgomery::new(&point_mont);

    // Start from most significant digit
    let mut result = PointMontgomery::infinity();

    for &digit in wnaf.iter().rev() {
        // Double for each bit (using Montgomery doubling - 45% faster!)
        result = result.double();

        // Add if non-zero (using Montgomery mixed addition - 15% faster!)
        if digit > 0 {
            // Positive: add digit×P using Montgomery mixed addition
            let affine_point = table.lookup(digit as usize);
            result = result.add_affine(affine_point);
        } else if digit < 0 {
            // Negative: subtract |digit|×P by adding its negation
    ...
    // Convert result back to standard form once
    result.to_point()
}

/// Precomputed table for Montgomery wNAF scalar multiplication
///
/// Stores odd multiples in Montgomery form but converts to affine for lookups
/// to enable fast mixed addition.
pub struct WNafTableMontgomery {
    /// Odd multiples in affine coordinates: [P, 3P, 5P, ..., (2^w - 1)P]
    /// Stored in affine form for mixed addition
    table: [AffinePoint; MAX_WNAF_VALUE / 2],
}

impl WNafTableMontgomery {
    /// Create a new wNAF table for Montgomery scalar multiplication
    ///
    /// # Algorithm
    ///
    /// 1. Compute 2P in Montgomery form (one doubling)
    /// 2. Generate odd multiples: P, P+2P, (P+2P)+2P, ...
    /// 3. Convert batch to affine using batch inversion
    /// 4. Store as affine points for mixed addition
    ///
    /// This uses Montgomery arithmetic for the computation but stores results
    /// in affine form for fast mixed addition during scalar multiplication.
    pub fn new(point_mont: &super::PointMontgomery) -> Self {
        // Compute 2P in Montgomery form
        let two_p_mont = point_mont.double();
        ...
        let mut table = [AffinePoint {
            x: super::FieldElement::zero(),
            y: super::FieldElement::zero(),
        }; MAX_WNAF_VALUE / 2];

        for (i, (point, z_inv)) in multiples_standard.iter().zip(z_coords.iter()).enumerate() {
            let z2_inv = z_inv.square();
            let z3_inv = z2_inv.mul(z_inv);

            table[i] = AffinePoint {
                x: point.x.mul(&z2_inv),
                y: point.y.mul(&z3_inv),
            };
        }
    ...
    /// Lookup an odd multiple from the table
    ///
    /// # Arguments
    ///
    /// * `value` - An odd number: 1, 3, 5, ..., (2^w - 1)
    ///
    /// # Returns
    ///
    /// The affine point corresponding to value × P
    #[inline]
    pub fn lookup(&self, value: usize) -> &AffinePoint {
        debug_assert!(value > 0 && value <= MAX_WNAF_VALUE && value % 2 == 1);
        let index = (value - 1) / 2;
        &self.table[index]
    }
}
