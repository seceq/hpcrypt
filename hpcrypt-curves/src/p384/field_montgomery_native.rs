//! Native High-Performance Montgomery Field Arithmetic for P-384
//!
...
//! # Performance Goals
//!
//! Target: Similar speedup as P-256 (37x for single ops, 100-130x for batch operations)
//!
//! # Algorithm
//!
...
//! - Optimized for 6-limb (384-bit) arithmetic
//!
//! # References
//!
...
const MONTGOMERY_MU: u64 = 0x0000000100000001;

/// A field element in Montgomery form for P-384
///
/// Represented as `a * R mod p` where R = 2^384.
/// Uses 6 limbs of 64 bits each (little-endian).
...
impl MontgomeryFieldElement {
    ...
    #[inline(always)]
    pub const fn from_limbs_montgomery(limbs: [u64; 6]) -> Self {
        Self { limbs }
    }
    ...
    /// Computes: a * R mod p
    #[inline]
    pub fn to_montgomery(a: &[u64; 6]) -> Self {
    ...
    #[inline]
    pub fn from_montgomery(&self) -> [u64; 6] {
    ...
    /// - Manual loop unrolling for 6 limbs
    /// - Careful use of widening multiply (u64 × u64 → u128)
    #[inline(always)]
    fn montgomery_multiply(a: &[u64; 6], b: &[u64; 6]) -> Self {
        ...
        let mut t = [0u128; 7]; // Temporary storage (needs 7 limbs for overflow)
    ...
    pub fn to_bytes(&self) -> [u8; 48] {
        ...
        for i in 0..6 {
            let limb_bytes = normal[5 - i].to_be_bytes();
...
#[inline(always)]
fn gte_modulus_ct(limbs: &[u64; 6]) -> Choice {
    let mut borrow = 0u64;

    for i in 0..6 {
        let (diff, b1) = limbs[i].overflowing_sub(P384_MODULUS[i]);
...
/// Constant-time check if limbs < modulus
#[inline(always)]
fn lt_modulus_ct(limbs: &[u64; 6]) -> Choice {
...
#[inline(always)]
fn sub_modulus(limbs: &[u64; 6]) -> [u64; 6] {
    let mut result = [0u64; 6];
    let mut borrow = 0u64;
...
#[inline(always)]
fn add_modulus(limbs: &[u64; 6]) -> [u64; 6] {
    let mut result = [0u64; 6];
    let mut carry = 0u64;

    for i in 0..6 {
        let (sum, c1) = limbs[i].overflowing_add(P384_MODULUS[i]);
        let (sum, c2) = sum.overflowing_add(carry);
        result[i] = sum;
        carry = (c1 as u64) + (c2 as u64);
    }
