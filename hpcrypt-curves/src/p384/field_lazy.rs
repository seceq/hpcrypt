//! P-384 Field Arithmetic with Lazy Reduction
//!
//! This module implements lazy reduction for P-384 field operations.
...
//! # Representation
//!
//! Field elements are represented as 6 x 64-bit limbs (little-endian):
...
impl LazyFieldElement {
    ...
    #[inline(always)]
    pub const fn from_limbs_unchecked(limbs: [u64; 6]) -> Self {
        Self { limbs }
    }
    ...
    pub fn add_lazy(&self, rhs: &Self) -> Self {
        ...
        // If there's a final carry, we've exceeded 2^384
    ...
    pub fn sub_lazy(&self, rhs: &Self) -> Self {
        ...
        if borrow != 0 {
            let mut corrected = [0u64; 6];
            let mut carry = 0u64;

            for i in 0..6 {
                let (sum, c1) = result[i].overflowing_add(P384_MODULUS[i]);
                let (sum, c2) = sum.overflowing_add(carry);
                corrected[i] = sum;
                carry = (c1 as u64) + (c2 as u64);
            }
    ...
    #[inline(always)]
    fn needs_reduction(&self, limbs: &[u64; 6]) -> bool {
        // Compare with modulus
        for i in (0..6).rev() {
            if limbs[i] > P384_MODULUS[i] {
                return true;
            }
            if limbs[i] < P384_MODULUS[i] {
                return false;
    ...
    pub fn neg(&self) -> Self {
        ...
        for i in 0..6 {
            let (diff, b1) = P384_MODULUS[i].overflowing_sub(norm_limbs[i]);
            let (diff, b2) = diff.overflowing_sub(borrow);
            result[i] = diff;
            borrow = (b1 as u64) + (b2 as u64);
        }
...
mod tests {
    ...
    #[test]
    fn test_lazy_sub() {
        let five = LazyFieldElement::from_limbs_unchecked([5, 0, 0, 0, 0, 0]);
    ...
    fn test_normalization() {
        ...
        let p = LazyFieldElement::from_limbs_unchecked(P384_MODULUS);
        let one = LazyFieldElement::one();
