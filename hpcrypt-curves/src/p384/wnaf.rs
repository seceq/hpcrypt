...
/// - Default w=4: 8 odd multiples, ~512 bytes
/// - Feature scalar-mul-fast w=5: 16 odd multiples, ~1 KB, ~15-20% faster
/// - Feature scalar-mul-very-fast w=6: 32 odd multiples, ~2 KB, ~20-25% faster
...
pub fn compute_wnaf(scalar: &[u8; 48], width: usize) -> Vec<i8> {
    ...
    // Maximum length is 385 (384 bits + 1 potential carry bit)
    let mut wnaf = [0i8; 385];

    // Convert scalar bytes to mutable working copy
    // Scalar is in big-endian format: byte[0] is MSB, byte[47] is LSB
    // We need to convert to little-endian u64 limbs: k[0] is LSW, k[5] is MSW
    let mut k = [0u64; 6];
    for i in 0..6 {
        // Read bytes in big-endian order, but store limbs in little-endian order
        // byte[47-7..47] -> k[0], byte[39..32] -> k[1], etc.
        let byte_start = 40 - (i * 8);  // 40, 32, 24, 16, 8, 0
...
pub fn wnaf_scalar_mul(point: &Point, scalar: &[u8; 48], width: usize) -> Point {
...
#[inline]
fn sub_small(k: &mut [u64; 6], val: u64) {
...
#[inline]
fn add_small(k: &mut [u64; 6], val: u64) {
...
mod tests {
    ...
    fn test_wnaf_simple() {
        ...
        scalar[47] = 1;  // Value 1 in big-endian (LSB is at byte[31])
