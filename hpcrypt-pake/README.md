# hpcrypt-pake - Password-Authenticated Key Exchange

High-performance Password-Authenticated Key Exchange (PAKE) protocols implemented in pure Rust.
...
```rust
use hpcrypt_pake::opaque::{OpaqueClient, OpaqueServer, Config};
...
```rust
use hpcrypt_pake::opaque::{ServerKeyStorage, OpaqueError};
...
```rust
use hpcrypt_pake::opaque::InMemoryStorage;
...
```bash
# Run fast tests
cargo test -p hpcrypt-pake

# Run slow tests (Argon2id with production parameters)
cargo test -p hpcrypt-pake -- --ignored
```
