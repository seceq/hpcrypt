...
impl Params {
    ...
    pub fn new(outlen: usize, mem_cost: u32, time_cost: u32, lanes: u32) -> Result<Self, KdfError> {
        ...
        Ok(Self { outlen, mem_cost, time_cost, lanes })
    }
}
...
impl Argon2 {
    ...
    pub fn hash_with_ad(
        ...
        let mut memory = vec![vec![0u64; QWORDS_IN_BLOCK]; self.params.lanes as usize * lane_length];
        ...
        let mut final_block = memory[(self.params.lanes as usize - 1) * lane_length + lane_length - 1].clone();
